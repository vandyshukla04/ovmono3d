...
def main():
    ...
    # Write a matching configs/category_meta_<N>species.json so that the
    # evaluator at TEST.CAT_MODE="novel" sees the SAME contiguous-id ordering
    # that cubercnn/data/datasets.py:register_and_store_model_metadata() will
    # generate at training time (sorted ascending by dataset-id). This is the
    # #1 source of silent "per-class metrics show as 0 for all but one class"
    # bugs at eval time. See WILDBOX_EXPERIMENT.md §2.8.
    if sources:
        # Build the mapping from the actual sources we used (not pending ones)
        species_by_id = {}
        for s in sources:
            species_by_id[s.category_id] = s.category_name
        sorted_ids = sorted(species_by_id.keys())
        thing_classes = [species_by_id[i] for i in sorted_ids]
        id_map = {str(sorted_ids[i]): i for i in range(len(sorted_ids))}
        n = len(sorted_ids)
        meta_out = Path(f"configs/wildbox/category_meta_auto_{n}species.json")
        try:
            meta_out.parent.mkdir(parents=True, exist_ok=True)
            with open(meta_out, "w") as f:
                json.dump({
                    "_comment": (
                        f"Auto-generated by prepare_wildbox_dataset.py at "
                        f"{__import__('datetime').date.today()}. Mapping matches "
                        f"register_and_store_model_metadata()'s sort-by-dataset-id "
                        f"convention. Symlink configs/category_meta.json at this file "
                        f"(or edit by hand) before running eval."
                    ),
                    "thing_classes": thing_classes,
                    "thing_dataset_id_to_contiguous_id": id_map,
                }, f, indent=2)
            logger.info(
                f"Wrote category meta → {meta_out}. "
                f"To activate: ln -sf wildbox/{meta_out.name} configs/category_meta.json"
            )
        except Exception as e:
            logger.warning(f"Could not write auto category meta: {e}")
